document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM fully loaded. Initializing application features. Implementing Add Tag.');

    // Cache DOM elements (ensure all relevant ones are cached)
    const photoWall = document.getElementById('photo-wall');
    const prevPageBtn = document.getElementById('prev-page');
    const nextPageBtn = document.getElementById('next-page');
    const pageInfoSpan = document.getElementById('page-info');
    const orgPathsList = document.getElementById('org-paths-list');
    const globalTagsList = document.getElementById('global-tags-list'); // For right info panel
    ...
    const tagManagementModal = document.getElementById('tag-management-modal'); // Ensure this modal exists
    const newTagInput = document.getElementById('new-tag-input');
    const addNewTagBtn = document.getElementById('add-new-tag-btn');
    const manageTagsListUl = document.getElementById('manage-tags-list'); // UL for tags in modal
    ...
    // --- Minified/Unchanged functions for brevity (assuming they are correct from previous steps) ---
    ...
    async function fetchGlobalTags() { /* For info panel */ try{const r=await fetch('/api/tags');const d=await r.json();globalTagsList.innerHTML='';d.forEach(t=>{const l=document.createElement('li');l.textContent=t.name;l.dataset.tagId=t.id;globalTagsList.appendChild(l)})}catch(e){console.error('Error fetching global tags for info panel:',e)} }

    // --- Tag Management Modal: Populate List & Add Tag ---
    async function populateManageTagsList() {
        console.log('[TagManagement] Populating manage tags list...');
        if (!manageTagsListUl) {
            console.error("[TagManagement] manageTagsListUl element not found.");
            return;
        }
        try {
            const response = await fetch('/api/tags'); // GET request by default
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`HTTP error! Status: ${response.status}, Message: ${errorText}`);
            }
            const tags = await response.json();
            manageTagsListUl.innerHTML = ''; // Clear existing list
            if (tags.length === 0) {
                manageTagsListUl.innerHTML = '<li>No tags defined yet.</li>';
            } else {
                tags.forEach(tag => {
                    ...
                    // deleteBtn.onclick = () => handleDeleteTag(tag.id, tag.name); // Function to be created
                    // li.appendChild(deleteBtn);
                    manageTagsListUl.appendChild(li);
                });
            }
            console.log('[TagManagement] Manage tags list populated with', tags.length, 'tags.');
        } catch (error) {
            console.error('Error populating manage tags list:', error);
            manageTagsListUl.innerHTML = `<li>Error loading tags: ${error.message}</li>`;
        }
    }
    ...
    if (addNewTagBtn) {
        addNewTagBtn.addEventListener('click', async () => {
            ...
            console.log(`[TagManagement] 'Add Tag' button clicked. Tag name: '${tagName}'`);

            if (!tagManagementStatusDiv) {
                console.error("[TagManagement] tagManagementStatusDiv element not found.");
                return;
            }
            tagManagementStatusDiv.textContent = ''; // Clear previous status
            tagManagementStatusDiv.style.color = ''; // Reset color

            if (!tagName) {
                tagManagementStatusDiv.textContent = 'Tag name cannot be empty.';
                tagManagementStatusDiv.style.color = 'red';
                return;
            }
            ...
            try {
                ...
                if (response.ok) { // Status 200-299
                    tagManagementStatusDiv.textContent = `Tag '${result.name}' processed successfully.`;
                    tagManagementStatusDiv.style.color = 'green';
                    newTagInput.value = ''; // Clear input field
                    populateManageTagsList(); // Refresh list in modal
                    fetchGlobalTags();      // Refresh list in info panel (right side)
                } else {
                    tagManagementStatusDiv.textContent = `Error: ${result.error || 'Could not add tag.'}`;
                    tagManagementStatusDiv.style.color = 'red';
                }
            } catch (error) {
                console.error('[TagManagement] Error adding tag:', error);
                tagManagementStatusDiv.textContent = 'Network error or server issue adding tag.';
                tagManagementStatusDiv.style.color = 'red';
            }
        });
    } else {
        console.warn("[TagManagement] 'add-new-tag-btn' not found.");
    }

    // --- Event Listeners for Menu Controls & Modals (rest are unchanged) ---
    ...
    if (tagManagementBtn) {
        tagManagementBtn.onclick = () => {
            console.log('[TagManagement] Tag management modal opened.');
            if (tagManagementStatusDiv) tagManagementStatusDiv.textContent = ''; // Clear status on open
            openModal('tag-management-modal');
            populateManageTagsList(); // Populate the list when modal is opened
        };
    } else {
        console.warn("[TagManagement] 'tag-management-btn' not found.");
    }

    // Initial Data Load
    fetchMedia(currentPage, currentSortBy, currentSortOrder);
    fetchOrgPaths();
    fetchGlobalTags(); // For info panel
