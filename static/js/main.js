document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentMediaItems = [], isXKeyPressed = false, isTKeyPressed = false, resizeTimeout, lastCalculatedPerPage = 0;

    document.addEventListener('keydown', (event) => { if(event.key==='x'||event.key==='X')isXKeyPressed=true; if(event.key==='t'||event.key==='T')isTKeyPressed=true; });
    document.addEventListener('keyup', (event) => { if(event.key==='x'||event.key==='X')isXKeyPressed=false; if(event.key==='t'||event.key==='T')isTKeyPressed=false; });
    ...
    if(refreshBtn) refreshBtn.addEventListener('click', async () => {const o=refreshBtn.textContent;refreshBtn.textContent='Scanning...';refreshBtn.disabled=true;let s=false;try{const r=await fetch('/api/scan/trigger',{method:'POST'});const t=await r.json().catch(()=>({error:"JSON Error"}));if(!r.ok){alert(`Scan Error: ${t.error||'Unknown'}`);s=true}else{alert('Scan complete.')}}catch(e){alert('Scan Network Error.');s=true}refreshBtn.textContent=o;refreshBtn.disabled=false;if(!s){clearSelectionsAndActiveTags();fetchMedia(1);if(fetchOrgPaths)fetchOrgPaths();if(fetchGlobalTags)fetchGlobalTags();if(tagManagementModal && tagManagementModal.style.display==='block' && populateManageTagsList)populateManageTagsList()}});
